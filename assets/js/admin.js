jQuery(function($) {
	...
	generate_apikey_form.on('submit', function(e) {
		...
		$.ajax({
			type: 'POST',
			url: location.protocol + '//' + location.hostname + ':' + location.port + '/wp-json/woocommerce_transafe/v1/generate_api_key',
			data: data, 
			showLoader: true,
			success: function(response) {
				...
				if (response.code === 'success') {
					...
					generate_apikey_form.get(0).reset();

					profile_select.prop('disabled', true);
					$('#transafe-profile-select-container').addClass('hidden');

					mfa_code_input.prop('disabled', true);
					$('#transafe-mfa-code-container').addClass('hidden');
